...
import { Plus, FileText, Users, Shield, BarChart3, RefreshCw, BrainCircuit } from 'lucide-react';
...
const actions = [
  ...
  { icon: BrainCircuit, label: 'Continuous Intelligence', href: '/intelligence' },
];
    ...
    <div className="assetflow-card rounded-2xl p-5 md:p-6">
      ...
      <div className="mt-5 grid grid-cols-2 gap-2.5">
        {actions.map((action, index) => (
          <button key={index} onClick={() => action.href && (window.location.href = action.href)} className="group flex min-h-[92px] flex-col items-start justify-between rounded-xl border border-white/[.07] bg-white/[.025] p-3.5 text-left transition duration-200 hover:-translate-y-0.5 hover:border-cyan-300/20 hover:bg-white/[.05] focus:outline-none focus:ring-2 focus:ring-cyan-300/20">
