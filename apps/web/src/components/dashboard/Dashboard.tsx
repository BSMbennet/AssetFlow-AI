  ...
  return (
    <div className="assetflow-shell flex h-screen overflow-hidden">
      <aside className="assetflow-sidebar hidden w-64 shrink-0 flex-col border-r md:flex">
        <div className="border-b border-white/10 px-6 py-6"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br from-cyan-300 via-cyan-400 to-violet-500 text-slate-950 shadow-lg shadow-cyan-500/20"><Sparkles className="h-5 w-5" /></div><div><h1 className="assetflow-brand text-xl font-bold tracking-tight">AssetFlow AI</h1><p className="text-[11px] uppercase tracking-[.18em] text-slate-500">Private Credit OS</p></div></div></div>
        <nav className="flex-1 space-y-1 px-3 py-5"><p className="px-3 pb-2 text-[10px] font-semibold uppercase tracking-[.2em] text-slate-600">Workspace</p>{navigation.map((item) => <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex w-full items-center rounded-xl px-3 py-3 text-sm transition-all ${activeTab === item.id ? 'assetflow-nav-active' : 'text-slate-500 hover:bg-white/[.04] hover:text-white'}`}><item.icon className="mr-3 h-4 w-4" />{item.label}</button>)}</nav>
        <div className="border-t border-white/10 p-3"><button onClick={signOut} className="flex w-full items-center rounded-xl px-3 py-3 text-sm text-slate-500 transition hover:bg-white/[.04] hover:text-white"><LogOut className="mr-3 h-4 w-4" /> Sign out</button></div>
      </aside>

      <main className="min-w-0 flex-1 overflow-auto">
        <header className="assetflow-topbar sticky top-0 z-20 border-b"><div className="flex items-center justify-between gap-4 px-4 py-3 md:px-8"><div className="flex min-w-0 flex-1 items-center gap-3"><div className="md:hidden"><div className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-cyan-300 to-violet-500 text-slate-950"><Sparkles className="h-4 w-4" /></div></div><div className="relative hidden w-full max-w-xl sm:block"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-600" /><input placeholder="Search assets, borrowers, documents..." className="assetflow-search w-full rounded-xl py-2.5 pl-10 pr-4 text-sm outline-none transition focus:border-cyan-400/40 focus:ring-2 focus:ring-cyan-400/10" /></div></div><div className="flex items-center gap-2"><button className="rounded-xl p-2.5 text-slate-500 transition hover:bg-white/[.05] hover:text-white"><Bell className="h-4 w-4" /></button><button onClick={() => setShowCreate(true)} className="assetflow-primary flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold transition hover:scale-[1.02]"><Plus className="h-4 w-4" /> <span className="hidden sm:inline">New Asset</span></button></div></div></header>
        ...
        {activeTab === 'compliance' ? <Phase3Workspace /> : activeTab === 'monitoring' ? <PortfolioMonitoringWorkspace /> : activeTab === 'tokens' ? <Phase5TokenizationWorkspace /> : activeTab === 'trading' ? <Phase6TradingWorkspace /> : <div className="mx-auto max-w-[1500px] space-y-6 p-4 md:p-8">
          {activeTab === 'overview' && <><section className="assetflow-hero rounded-3xl p-6 md:p-8"><div className="assetflow-orb pointer-events-none" /><div className="relative z-10 max-w-2xl"><div className="assetflow-chip mb-4 inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs"><Sparkles className="h-3.5 w-3.5 text-cyan-300" /> AI asset intelligence</div><h2 className="max-w-xl text-3xl font-semibold tracking-tight text-white md:text-5xl">Turn private credit data into <span className="bg-gradient-to-r from-cyan-300 via-violet-300 to-fuchsia-300 bg-clip-text text-transparent">actionable intelligence.</span></h2><p className="assetflow-muted mt-4 max-w-xl text-sm leading-6 md:text-base">Monitor assets, documents, investors and compliance from one intelligent workspace.</p><div className="mt-6 flex flex-wrap gap-2"><span className="assetflow-chip rounded-full px-3 py-1.5 text-xs">Live portfolio</span><span className="assetflow-chip rounded-full px-3 py-1.5 text-xs">Supabase connected</span><span className="assetflow-chip rounded-full px-3 py-1.5 text-xs">AI-ready</span></div></div></section><StatsCards /><div className="grid grid-cols-1 gap-6 lg:grid-cols-3"><div className="lg:col-span-2"><AssetChart /></div><div><QuickActions /></div></div><ActivityFeed /></>}
