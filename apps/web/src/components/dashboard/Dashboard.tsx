...
export function Dashboard() {
  ...
  return (
    <div className="assetflow-shell flex h-screen overflow-hidden">
      {mobileNavOpen && <div onClick={() => setMobileNavOpen(false)} className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm md:hidden" />}

      <aside className={`assetflow-sidebar fixed inset-y-0 left-0 z-50 flex w-72 shrink-0 flex-col border-r transition-transform duration-300 ease-out md:static md:z-auto md:w-64 md:translate-x-0 md:flex ${mobileNavOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="border-b border-white/10 px-6 py-6">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="grid h-10 w-10 place-items-center rounded-2xl bg-gradient-to-br from-cyan-300 via-cyan-400 to-amber-500 text-slate-950 shadow-lg shadow-cyan-500/20"><Sparkles className="h-5 w-5" /></div>
              <div><h1 className="assetflow-brand font-display text-xl font-bold tracking-tight">AssetFlow AI</h1><p className="text-[11px] uppercase tracking-[.18em] text-slate-500">Private Credit OS</p></div>
            </div>
        ...
        <nav className="flex-1 space-y-1 px-3 py-5">
          ...
          {navigation.map((item) => (
            <button key={item.id} onClick={() => { setActiveTab(item.id); setMobileNavOpen(false); }} className={`flex w-full items-center rounded-xl border-l-2 px-3 py-3 text-sm transition-all ${activeTab === item.id ? 'assetflow-nav-active' : 'border-transparent text-slate-500 hover:bg-white/[.04] hover:text-white'}`}>
