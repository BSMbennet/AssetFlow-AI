...
export function HumanApprovalWorkspace() {
  ...
  async function load() {
    ...
    const nextError = assetError?.message ?? recordError?.message ?? null;
    ...
    return (
      <section className="rounded-2xl border border-amber-400/15 bg-amber-400/[0.04] p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-amber-200">Human review data is temporarily unavailable.</p>
            <p className="mt-1 break-words text-xs leading-5 text-white/45">{loadError.includes('schema cache') ? 'The database table exists, but the Supabase API schema cache has not refreshed yet. Reload the page after the schema refresh completes.' : loadError}</p>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-5 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-cyan-400/10 text-cyan-300"><ShieldCheck className="h-5 w-5" /></div><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300">Milestone 3</p><h2 className="mt-1 text-xl font-semibold">Human approval & audit trail</h2></div></div>
        <select value={selectedAssetId} onChange={(event) => setSelectedAssetId(event.target.value)} className="w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm text-white outline-none md:w-auto">
