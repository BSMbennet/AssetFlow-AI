...
type Action = {
  id: string; asset_id: string; action_type: string; amount: number; currency: string;
  record_date: string; payment_date: string; status: string; notes: string | null;
  external_reference: string | null; created_at: string;
};

type Allocation = {
  id: string; corporate_action_id: string; investor_id: string; units: number; amount: number;
  status: string; paid_at: string | null; external_reference: string | null; transaction_hash: string | null;
};
...
const statuses = ['DRAFT', 'SCHEDULED', 'READY', 'PROCESSING', 'COMPLETED', 'FAILED', 'CANCELLED'];
...
export function Phase8ServicingWorkspace() {
  ...
  const selectedPositions = selected ? positions.filter(p => p.asset_id === selected.asset_id) : [];
  ...
  const upcoming = actions.filter(a => ['SCHEDULED', 'READY'].includes(a.status)).length;
  const processing = actions.filter(a => a.status === 'PROCESSING').length;
  ...
  async function createAction() {
    ...
    const { data, error } = await supabase.from('corporate_actions').insert({
      organization_id: profile.organization_id, asset_id: assetId, created_by: userData.user.id,
      action_type: actionType, amount: value, currency: 'USD', record_date: recordDate,
      payment_date: paymentDate, status: 'SCHEDULED', notes: notes.trim() || null,
    }).select().single();
