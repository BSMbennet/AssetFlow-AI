...
import {
  ...
  Scale, ShieldCheck, Sparkles, Users, WalletCards, XCircle
} from 'lucide-react';
...
const modules: Module[] = [
  ...
  { phase: 28, name: 'Multi-Rail Payments', table: 'payment_transfers', icon: CircleDollarSign, description: 'Idempotent payment instructions across financial rails.', seed: { rail: 'BANK', amount: 0, currency: 'USD', status: 'PENDING', idempotency_key: `control-plane-${Date.now()}` } },
...
export default function InstitutionalControlPlaneWorkspace() {
  ...
  const [samples, setSamples] = useState<Record<string, Row[]>>({});
  ...
  async function load() {
    ...
    const nextSamples: Record<string, Row[]> = {};
    await Promise.all(modules.map(async m => {
      const { data, count, error: e } = await supabase.from(m.table).select('*', { count: 'exact' }).eq('organization_id', profile.organization_id).order('created_at', { ascending: false }).limit(4);
      if (!e) { nextCounts[m.table] = count || 0; nextSamples[m.table] = data || []; }
    }));
    setCounts(nextCounts); setSamples(nextSamples); setLoading(false);
  }
  ...
  async function initialize(m: Module) {
    ...
    const payload = { ...m.seed, organization_id: orgId };
    if (m.table === 'copilot_sessions') payload.user_id = (await supabase.auth.getUser()).data.user?.id || null;
  ...
  const filtered = useMemo(() => modules.filter(m => `${m.phase} ${m.name} ${m.description}`.toLowerCase().includes(query.toLowerCase())), [query]);
    ...
    <main className="mx-auto max-w-[1540px] space-y-6 p-4 md:p-8">
      <section className="assetflow-command rounded-3xl p-6 md:p-8">
        <div className="relative z-10">
          <div className="mb-3 flex flex-wrap items-center gap-2"><span className="assetflow-eyebrow">Phases 16–35 · Unified Institutional Control Plane</span><span className="assetflow-status"><span className="assetflow-live-dot"/> Organization isolated</span></div>
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div><h1 className="font-display text-3xl font-semibold tracking-[-.04em] text-white md:text-5xl">One operating system. Every remaining phase.</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">Evidence, policy, data rooms, counterparties, valuation, treasury, risk, compliance, automation, AI context, distribution, custody, payments, reconciliation, analytics, security, network, regulation, liquidity and bounded autonomy — wired into one institutional workspace.</p></div>
            <button onClick={load} className="inline-flex items-center gap-2 self-start rounded-xl border border-cyan-300/20 bg-cyan-300/5 px-4 py-3 font-mono text-[10px] uppercase tracking-[.16em] text-cyan-200"><RefreshCw className="h-3.5 w-3.5"/> Refresh control plane</button>
          </div>
        </div>
      </section>
      ...
      <section className="assetflow-panel rounded-2xl p-5 md:p-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="assetflow-eyebrow">Institutional topology</p><h2 className="mt-1 font-display text-xl font-semibold text-white">Phase execution map</h2></div><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Filter phase, domain or capability…" className="w-full rounded-xl border border-white/10 bg-white/[.03] px-4 py-3 text-xs text-slate-200 outline-none placeholder:text-slate-600 md:w-80" /></div>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {filtered.map(m => <ModuleCard key={m.phase} module={m} count={counts[m.table] || 0} rows={samples[m.table] || []} loading={loading} busy={busy === m.table} onInitialize={() => initialize(m)} />)}
      </div>
